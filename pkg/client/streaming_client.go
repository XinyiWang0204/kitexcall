...
func (c *StreamingClient) handleServerStreaming(ctx context.Context) error {
	...
	// Track if stream is closed by server
	streamClosed := make(chan struct{})

	// Only start goroutine for user cancellation in interactive mode
	if c.Conf.File == "" {
		// Start a goroutine to handle user cancellation
		go func() {
			// Wait for Ctrl+D (EOF) from stdin
			_, err := c.stream.Recv()
			if err == io.EOF {
				if !c.Conf.Quiet {
					log.Info("Stream cancelled by user")
				}
				// Signal that user wants to stop the stream
				close(streamClosed)
			}
		}()
	}
			...
			if err != nil {
				if err == io.EOF {
					...
					if c.Conf.File == "" {
						...
						// Only close streamClosed if it hasn't been closed yet
						closeOnce.Do(func() {
							close(streamClosed)
						})
					}
