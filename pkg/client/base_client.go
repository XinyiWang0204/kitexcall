...
func (c *BaseClient) buildClientOptions() error {
	...
	if c.Conf.IsStreaming {
		...
		// Only warn if user explicitly specified a non-GRPC transport
		if c.Conf.Transport != "" && c.Conf.Transport != "GRPC" && c.Conf.Transport != config.GRPC {
			log.Warnf("Streaming calls require GRPC transport. Your specified transport '%s' will be ignored.\n", c.Conf.Transport)
		}
